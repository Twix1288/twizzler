...
impl HiArgs {
    ...
    pub(crate) fn from_low_args(mut low: LowArgs) -> anyhow::Result<HiArgs> {
        ...
        let threads = if low.sort.is_some() || paths.is_one_file {
            ...
            let n = std::thread::available_parallelism().map_or(1, |n| n.get()).min(12);
            ...
            let n = {
                ...
                unsafe extern "C" {
                    fn twz_rt_get_sysinfo() -> system_info;
                }
                unsafe { twz_rt_get_sysinfo().available_parallelism }.max(1).min(12)
            };
            n
        };
...
impl State {
    ...
    fn new() -> anyhow::Result<State> {
        ...
        Ok(State {
            is_terminal_stdout: ({ #[cfg(not(target_os = "twizzler"))] { std::io::stdout().is_terminal() } #[cfg(target_os = "twizzler")] { false } }),
